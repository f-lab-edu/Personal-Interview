...
class EmailVerifyTest {
	...
	void create_Success() {
		...
		// when
		EmailVerify result = EmailVerify.create(userId, authProperties);
		...
		assertAll(
			...
			() -> assertThat(result.getSendCount()).isEqualTo(1),

			() -> assertThat(result.getFirstAt()).isAfterOrEqualTo(now),
			() -> assertThat(result.getExpiredAt()).isEqualToIgnoringNanos(result.getFirstAt().plusMinutes(authProperties.verificationExpirationMinutes())),
			() -> assertThat(result.getCountResetAt()).isEqualToIgnoringNanos(result.getFirstAt().plusHours(authProperties.countResetHours()))
		);
	}
	...
	void reCreate_Success() {
		...
		EmailVerify original = EmailVerify.create(new UserId(1L), authProperties);
		LocalDateTime previousExpiry = original.getExpiredAt();

		// when
		EmailVerify result = original.reCreate(authProperties);

		// then
		assertAll(
			() -> assertThat(result.getSendCount()).isEqualTo(2),
	...
	void reCreate_Fail_AlreadyVerified() {
		...
		// when & then
		assertThatThrownBy(() -> original.reCreate(authProperties))
	...
	void reCreate_Fail_ExceedLimit() {
		// given: 발송 횟수를 5로 설정
		EmailVerify original = EmailVerify.create(new UserId(1L), authProperties);
		ReflectionTestUtils.setField(original, "sendCount", 5);
	...
	void reCreate_ResetCount_WhenTimePassed() {
		...
		ReflectionTestUtils.setField(original, "sendCount", 3);
	...
	void verify_Success() {
		// given
		EmailVerify emailVerify = EmailVerify.create(new UserId(1L), authProperties);
